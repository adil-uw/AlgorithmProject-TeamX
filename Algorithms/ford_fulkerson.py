...
class Edge:
    ...
    Residual edge structure:
        to       – neighbor node
        capacity – current residual capacity
...
def addEdge(res: List[List[Edge]], u: int, v: int, capacity: int) -> None:
    """
    Add a residual forward edge u -> v (capacity = capacity)
    and residual backward edge v -> u (capacity = 0).
    """
...
def buildResidualGraph(n: int, edges: List[EdgeTuple]) -> List[List[Edge]]:
    """
    Builds the initial residual graph from the given list of edges.
    """
...
def dfs(u: int,
    ...
    """
    Depth-first search to push flow from u to t, up to flow_limit.
    Returns the amount of flow actually pushed.
    """
    if u == t:
        return flow_limit
...
def fordFulkMaxFlow(n: int,
    ...
    Returns the integer max-flow value.
    """
